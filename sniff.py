...
# code comes from here http://www.binarytides.com/python-packet-sniffer-code-linux/
...
def cleanIps(ips):
    return filter(lambda obj: datetime.datetime.now() - obj['date'] < datetime.timedelta(milliseconds=MAX_AGE_MS), ips)
...
while True:
    ...
    if eth_protocol == 8 :
        ...
        addIp(str(s_addr), ips)
        addIp(str(d_addr), ips)
	ips = cleanIps(ips)
        print(chr(27) + "[2J")
        #print('\nDest IPs\n' + '\n'.join(destIps) + '\n\nSource IPs\n' + '\n'.join(sourceIps) + '\n*******************\n')
        print('\n'.join(map(lambda obj: obj['ip'], ips)) + '\n*******************\n')
